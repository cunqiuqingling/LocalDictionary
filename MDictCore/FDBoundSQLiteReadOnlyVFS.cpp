...
namespace localdict::fdsqlite {
namespace {

constexpr char kVFSName[] = "localdict_fd_readonly_v1";
constexpr char kTokenPrefix[] = "/localdict-fd-vfs-";
...
FDBoundFileIdentity IdentityFromStat(const struct stat &value) {
  return {value.st_dev,
          ...
          static_cast<mode_t>(value.st_mode & 0777),
          value.st_uid,
          value.st_nlink,
          value.st_size};
}
...
bool SameObject(const FDBoundFileIdentity &left,
  ...
  if (left.device != right.device || left.inode != right.inode ||
      left.type != right.type || left.permissions != right.permissions ||
...
void VFSDLError(sqlite3_vfs *, int size, char *message) {
  if (size > 0 && message) {
    sqlite3_snprintf(size, message,
                     "dynamic loading disabled by fd-bound read-only VFS");
  }
}
...
int FDBoundReadOnlyFileCapability::descriptor() const {
  return descriptor_;
}
...
int EnsureFDBoundReadOnlyVFSRegistered() {
  ...
  const int result = sqlite3_vfs_register(&ReadOnlyVFS(), 0);
...
int ShutdownFDBoundReadOnlyVFS() {
  ...
  const int result = sqlite3_vfs_unregister(&ReadOnlyVFS());
