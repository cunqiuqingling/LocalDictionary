...
class Mdict {
 public:
  ...
  /** Initialize block metadata without decoding all headwords. */
  void initMetadataOnly();

  /** Read one MDX record by its global decompressed byte range. */
  std::string readRecordAt(uint64_t record_start, uint64_t record_end);

  uint64_t entryCount() const { return entries_num; }
  uint64_t recordStreamSize() const;
  float engineVersion() const { return version; }
  ...
  /** Decode key-block metadata but not the complete headword list. */
  void read_key_block_info_metadata();
 ...
 private:
  ...
  std::vector<uint8_t> readRecordBlockBytes(uint64_t block_id);
