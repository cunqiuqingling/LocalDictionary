...
ROOT="${0:A:h:h}"
BUILD="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/LocalDictionary-official-discovery.XXXXXX")"
...
xcrun --sdk macosx swiftc \
  ...
  "$ROOT/Tests/OfficialOpenResourceDiscoverySmoke.swift" \
  -o "$BUILD/OfficialOpenResourceDiscoverySmoke"

"$BUILD/OfficialOpenResourceDiscoverySmoke" "$@"
