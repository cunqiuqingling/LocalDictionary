...
ROOT="${0:A:h:h}"
BUILD="$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/LocalDictionary-production-network.XXXXXX")"
...
xcrun --sdk macosx swiftc \
  ...
  "$ROOT/Tests/ProductionOpenResourceNetworkProbe.swift" \
  -o "$BUILD/ProductionOpenResourceNetworkProbe"

"$BUILD/ProductionOpenResourceNetworkProbe" "$@"
