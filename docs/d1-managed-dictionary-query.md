...
old generation, then advances generation exactly once. An install, index, removal, or
...
transition never clears a runtime while an old query lease is active; after that query releases
and fails the current-Catalog recheck, the stale dictionary runtime is evicted. fd-bound source
...
in-flight result returns, the query service rechecks current Catalog eligibility and generation;
disabled, removed, or transitioned descriptors cannot publish stale results. Misses and runtime
